...
function normalizeCompatibleDraft(rawDraft, currentPublishedFingerprint) {
  ...
  if (
    normalized.draft.lastPublishedFingerprint &&
...
function parseNodeTranslate(value) {
  const match = /translate\(\s*(-?\d+(?:\.\d+)?)\s*,\s*(-?\d+(?:\.\d+)?)\s*\)/.exec(value ?? '');
  if (!match) return { x: 0, y: 0 };
  return { x: Number(match[1]), y: Number(match[2]) };
}

function configFromRenderedNode(node) {
...
function graphFromRenderedDom(direction) {
  const nodeElements = [...document.querySelectorAll('[data-node-id]')]
    .filter((node) =>
      !node.getAttribute?.('data-port-role') &&
      !node.getAttribute?.('data-port-kind')
    );
  if (nodeElements.length === 0) return null;

  const nodes = nodeElements.map((node) => {
    const position = parseNodeTranslate(node.getAttribute?.('transform'));
    return {
      id: node.getAttribute?.('data-node-id') ?? '',
      type: node.getAttribute?.('data-node-type') || 'policy',
      policy: node.getAttribute?.('data-node-policy') || node.getAttribute?.('data-node-id') || '',
      config: configFromRenderedNode(node),
      x: position.x,
      y: position.y,
      width: NODE_WIDTH,
...
function htmlEl(tag, attrs = {}, children = []) {
  ...
  for (const child of children) {
    node.appendChild(
      child && typeof child === 'object' && 'nodeType' in child ? child : document.createTextNode(String(child)),
    );
  }
  return node;
}
...
function setStatus(msg, isError = false) {
  ...
  if (summary && msg) {
    ...
    summary.classList.toggle('text-danger', isError);
  }
  ...
  for (const edge of graph.edges ?? []) {
    ...
    edgeLayer.appendChild(svgEl('path', {
      ...
      dataset: {
        edgeId: edge.id,
        edgeFrom: edge.from,
        edgeFromPort: edge.fromPort ?? '',
        edgeTo: edge.to,
        edgeToPort: edge.toPort ?? '',
      },
    }));
    edgeLayer.appendChild(svgEl('path', {
      class: 'pipeline-edge',
...
function renderNode(group, node) {
  ...
  if (shouldRenderInputPort(node)) {
    ...
    nodeGroup.appendChild(svgEl('circle', {
      ...
      cy: input.y,
      r: 6,
      dataset: { nodeId: node.id, portRole: 'input', portKind: 'input', portName: 'in' },
      events: { pointerup: handlePointerUp },
    }));
  }
  ...
  for (const port of outputPortsFor(node)) {
    ...
    nodeGroup.appendChild(svgEl('circle', {
      ...
      events: {
        pointerdown: (event) => startWire(event, node.id, port.id),
      },
    }));
    nodeGroup.appendChild(svgEl('text', {
...
function ensureModalRoot(id) {
  ...
  document.getElementById('pipeline-workbench')?.appendChild(modal);
  return modal;
}
...
function openNodeSettingsModal(node) {
  ...
  const root = ensureModalRoot('node-settings-modal');
  if (!root) return;
      ...
      htmlEl('div', { className: 'workbench-modal-footer' }, [
        ...
        htmlEl('div', { className: 'modal-footer-actions' }, [
          ...
          htmlEl('button', {
            ...
            text: 'Apply',
            dataset: { modalAction: 'apply-settings' },
...
function renderPresets() {
  ...
  for (const name of Object.keys(PRESETS)) {
    container.appendChild(htmlEl('button', {
      ...
      events: {
        click: () => {
          const textarea = document.getElementById('playground-message-input');
...
function openPlaygroundModal(node = null) {
  ...
  const root = ensureModalRoot('playground-modal');
  if (!root) return;
      ...
      htmlEl('div', { className: 'playground-modal-grid' }, [
        htmlEl('div', { className: 'test-run-inputs' }, [
          ...
          htmlEl('label', {
            ...
            htmlFor: 'playground-message-input',
            text: 'Message',
          }),
          message,
          ...
          htmlEl('button', {
            ...
            className: 'btn btn-primary',
            text: '▷ Run',
            dataset: { modalAction: 'run-playground' },
...
async function loadPipelinesPage(options = {}) {
  ...
  try {
    ...
    const fallbackPipelines = serializeCurrentPipelines();
    pipelines.client = cloneValue(configData.pipelines?.client ?? fallbackPipelines.client);
    pipelines.server = cloneValue(configData.pipelines?.server ?? fallbackPipelines.server);
...
function setPaletteCollapsed(collapsed, persist = true) {
  ...
  if (toggle) {
    ...
    toggle.setAttribute('aria-label', collapsed ? 'Expand palette' : 'Collapse palette');
  }
  if (persist) {
...
export function initPipelinesPage() {
  ...
  const renderedGraph = graphFromRenderedDom(currentDirection);
  pipelines.client = [];
  pipelines.server = [];
  graphs = pipelinesToGraph(pipelines);
  if (renderedGraph) graphs[currentDirection] = renderedGraph;
  syncPipelinesFromGraphs();
  historyStates = initialDirectionHistoryState(graphs);
  activateDirectionState(currentDirection);
  publishedFingerprint = fingerprintPipelines(pipelines);
  ...
  render();
  initialLoadPromise = loadPipelinesPage();
}
