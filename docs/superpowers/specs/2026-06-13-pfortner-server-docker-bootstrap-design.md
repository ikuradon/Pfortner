...
process start
  ...
  -> config exists?
       yes -> load config -> normal mode
       no  -> create bootstrap config -> setup mode
```

bootstrap config は loader が扱える最小 config とする。ただし `server.upstream_relay` は未設定を表現できるように schema を拡張するか、setup mode 専用の `BootstrapConfig` として runtime config から分ける。

setup mode の動作:

- `/admin` は setup UI を表示する。
- setup UI は `upstream_relay` と最小 relay metadata を保存する。
- 保存時に `/data/config.yaml` を完成させる。
...
3. empty dataDir の setup mode と `/health` `setup_required` を追加する。
4. production config schema から env-owned fields を落とす。
5. Dockerfile と `deno.json` tasks を `src/server/main.ts` 基準に更新する。
6. `admin/` root UI source を `src/admin/ui/` へ移し、Fresh asset build path を更新する。
7. root `admin/` と `scripts/serve.ts` の production usage を削除する。
8. `docs/current-architecture.md` を最終構成へ更新する。
