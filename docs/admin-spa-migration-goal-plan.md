...
- `/admin/`, `/admin/connections`, `/admin/pipelines`, `/admin/playground`, `/admin/metrics`, `/admin/blocklist`, `/admin/config`, `/admin/logs` は同じ `AdminAppShell` HTML を返す。
...
- Blocklist: `#btn-refresh`, `#ip-input`, `#btn-add-ip`, `#ip-list-container`, `#ip-tbody`, `#pubkey-input`, `#btn-add-pubkey`, `#pubkey-list-container`, `#pubkey-tbody`
