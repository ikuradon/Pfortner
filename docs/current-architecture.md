...
- `admin/static/islands/PipelineWorkbench.js`: `deno task build:admin-assets` の生成 output。programmatic Fresh app が URL-addressed island chunk を必要とする間だけ残す compatibility artifact で、source of truth は `admin/islands/PipelineWorkbench.browser.tsx` と `admin/islands/pipeline/*` である。
