...
- `admin/islands/PipelineWorkbench.browser.tsx`: `PipelineWorkbench.tsx` を browser で mount するための adapter source。`scripts/build_admin_islands.ts` / `deno task build:admin-islands` で `/admin/static/islands/PipelineWorkbench.js` へ bundle できるが、現時点の production chunk はまだ旧 static controller のため、次の差し替え対象である。
...
- `admin/static/islands/PipelineWorkbench.js`: 現行 admin island bridge 用の static chunk。Fresh/Vite などの client bundle step を導入するまでは、ブラウザ側 workbench interaction をここで mount する。次の削減対象は、この file を手書き controller から `admin/islands/PipelineWorkbench.tsx` を mount/hydrate するだけの生成・互換 adapter に縮小すること。
